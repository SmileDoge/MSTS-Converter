/*
 * The code was taken from the openrails repository
 */
...
namespace TrainSim.Parsers
{
    public abstract class SBR : IDisposable
    {
        ...
        public static SBR Open(string filename)
        {
            ...
            {
                throw new Exception("Unrecognized header \"" + headerString + "\" in " + filename);
            }
    ...
    public class SBRException : Exception
    {
        ...
        {
            //Logger.Warn("{2} in {0}:byte {1}", sbr.Filename, sbr.InputStream.BaseStream.Position, message);
        ...
        {
            //Logger.Info("{2} in {0}:byte {1}", sbr.Filename, sbr.InputStream.BaseStream.Position, message);
        }
